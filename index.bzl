...
def _ts_proto_library_aspect(target, ctx):
    ...
    args.add("--plugin=protoc-gen-ts=%s" % (ctx.executable._protoc_gen_ts_bin.path))

    args.add("--ts_out=%s" % (paths.join(ctx.bin_dir.path, ctx.label.workspace_root)))

    # Set in descriptors
    descriptor_sets_paths = [desc.path for desc in target[ProtoInfo].transitive_descriptor_sets.to_list()]

    args.add("--descriptor_set_in=%s" % (ctx.configuration.host_path_separator.join(descriptor_sets_paths)))
...
""".format(
    ...
    ctx.actions.run(
        ...
        tools = [ctx.executable._protoc_gen_ts_bin, ctx.executable._protoc],
...
ts_proto_library_aspect = aspect(
    ...
    attr_aspects = ["deps"],
    fragments = ["grpc_package_name", "experimental_features"], 
